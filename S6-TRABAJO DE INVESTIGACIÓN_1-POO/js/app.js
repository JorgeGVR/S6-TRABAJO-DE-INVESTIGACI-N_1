//MENU
//creacion de menu para moviles mediante javascripc
...
function getData() {
  //Optenemos  los datos nombre, email y mensaje  y le guardamos en variables para optenr el valor de nuestro formulario
  var nombre = document.getElementById("nombre").value;
  var email = document.getElementById("email").value;
  var mensaje = document.getElementById("mensaje").value;

  /*
  verificamos  si los campos estanvacion o no  si estan vacios me lanza una alerta y no me deja proseguir
  si los campos  estan  con datos la condicion se cumple y procesa el html descrio por  los document.write()
  */

  if (nombre == "" || email == "" || mensaje == "") {
    alert("Los campos no han sido llenados correctamente ");
  } else if (validarEmail(email)) {
    // me  dirige al archivo  enviar html donde se imprimira nuestro html de enviar
    ...
    document.write("</div>");
    document.write(
      '<p class="frase4">Gracias por su inscripción, le<br/>saluda Jorge Guevara</p>'
    );
    document.write(
      '<p class="frase1"> ' +
        nombre +
